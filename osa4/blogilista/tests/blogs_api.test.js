...
describe('when there is initially some blogs saved', () => {
  ...
  //Estetään testien alkamista ennen alustuksen loppumista käyttämällä Promise.all
  beforeEach(async () => {
    await Blog.deleteMany({})
    //await Blog.insertMany(helper.initialBlogs)
  
    //Taulukko blogiobjekteista helpperin avulla
    const blogObjects = helper.initialBlogs
      .map(blog => new Blog(blog))
  
    //Taulukko promiseista, kun tallennetaan taulukon blogit
    const promiseArray = blogObjects.map(blog => blog.save())
  
    //Koostetaan taulukollinen promiseja yhdeksi promiseksi ja odotetaan, etä se valmistuu
  ...
  */
  test('4.8: all blogs are returned and are JSON', async () => {
